...
(function () {
  ...
  const smallIcon =
    "https://lh3.google.com/u/0/d/1bk9O_nTyynwDA-gaafz-tox-FT8F2CNx=w1920-h947-iv1";
  const themePage =
    "https://d1lss44hh2trtw.cloudfront.net/assets/article/2021/09/09/spider-man-2-will-pit-miles-peter-against-venom-on-ps5-in-2023_feature.jpg";
  ...
  const dictTopics = {
    1: "Тема зачтена. Интересная тема для выбора, жду следующего шага. Успехов.)",
  ...
  if (paramsString.includes("/admin")) {
    ...
    const currCheckList = document.querySelectorAll(
      ".form-group.col-md-6.p-r-0"
    );
    ...
    const currMessText = document.querySelectorAll(
    ...
    if (paramsString.includes("commenttreehomeworkresult") && currMessText[1]) {
      ...
      const currElemSomeNice = document.querySelectorAll(
        ".help-block.sonata-ba-field-widget-help"
      );
      ...
      if (checkName(pureName)) {
        elemText(
          newElemText,
          " Возможно у участника неправильно поставлено имя."
        );
      }
    }
    ...
    const currPreset = document.querySelectorAll(
    ...
    if (currList.length === 8) {
      if (
        currList[6].textContent.trim() ===
        "Ссылка на видеоразбор домашней работы"
      ) {
    ...
    const textTopicArr = [
      706, 707, 708, 750, 751, 752, 753, 754, 755, 756, 757, 760, 761, 762, 763,
      764, 765, 766, 767, 768, 769, 770, 771, 772, 773, 774, 775, 776, 777, 778,
      779, 780, 781, 782, 783, 784, 785, 786, 787, 788, 822, 823, 824, 825, 826,
      827, 828, 829,
    ]; //Все текстовые занятия
    const notMainTopics = [
      8, 9, 10, 11, 12, 13, 14, 15, 32, 44, 49, 50, 52, 53, 54, 61, 62, 63, 64,
      65, 66, 67, 68, 69, 70, 79, 80, 81, 82, 88, 89, 90, 91, 92, 93, 94, 95,
      96, 97, 98, 99, 100, 101, 102, 103, 111, 112, 113, 114, 115, 116, 119,
      120, 121, 122, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136,
      137, 138, 139, 140, 141, 151, 152, 153, 154, 155, 156, 157, 158, 162, 163,
      164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, 175, 176, 177, 178,
      179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193,
      194, 195, 197, 198, 199, 200, 203, 204, 205, 206, 207, 208, 209, 210, 211,
      212, 213, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223, 224, 225, 226,
      227, 228, 229, 230, 435, 436, 437, 438, 439, 440, 441, 442, 443, 444, 445,
      446, 447, 448, 449, 450, 504, 505, 506, 507, 508, 509, 510, 511, 512, 513,
      713, 714, 715, 716, 717, 718, 719, 720, 721, 722, 723, 724, 725, 726, 729,
      733, 734, 735, 736, 737, 739, 740, 741, 791, 792, 793, 794, 798, 800, 802,
      804, 805, 808, 810, 811, 812, 813, 818, 819, 820, 821, 830, 833, 836, 838,
      842, 843, 845, 847, 848, 849, 851, 867, 877, 878, 879, 880, 881, 882, 883,
      884,
    ]; // Уроки не из основной программы

    const graduateTopics = [
      238, 240, 241, 242, 243, 244, 245, 246, 247, 743, 744, 745,
    ]; // Все дипломные занятия.

    const namesTopicArr = [
      "гринь александр витальевич",
      "чаевский алексей валерьевич",
      "попов дмитрий владимирович",
      "буньков александр андреевич",
      "миних владимир анатольевич",
      "кротов никита михайлович",
      "корепин павел александр",
      "еремин евгений александрович",
      "тухватуллин руслан рафикович",
      "ли анатолий  евгеньевич",
    ];
    ...
    if (currLinks) {
      ...
      const currNameText = currName[0].textContent
        .trim()
        .split(":")[1]
        .trim()
        .toLowerCase();
      ...
      if (
        (notMainTopics.includes(currLinkText) ||
          textTopicArr.includes(currLinkText)) &&
        !namesTopicArr.includes(currNameText)
      ) {
        resText = funcName(pureName, regularText);
      }
    }
    ...
    const currTopic = document.querySelectorAll(
   ...
   color:${textColor};
    ...
    currGroup.forEach((e) => {
      if (i + 1 === currGroup.length) {
        ...
        if (
          currGroup[i].children[0].textContent.trim().toLowerCase() ===
          "фильтровать"
        ) {
    ...
    if (document.querySelector("div.form-group a.btn.btn-default")) {
      document.querySelector("div.form-group a.btn.btn-default").style.display =
        "none";
    }
        ...
        if (currLinks.children.length) {
          for (let i = 0; i < currLinks.children.length; i++) {
            currLinks.children[i].addEventListener("mouseover", (e) => {
              const currSomeText =
                e.currentTarget.firstChild.textContent.trim();
              if (currSomeText.toLowerCase() !== "сдал") {
                currNum[1].value = 0;
                currPreset[2].children[0].textContent = funcName(
                  pureName,
                  extraMess
                );
              } else {
...
currAAA    }`);
    ...
    setInterval(() => {
      const allButtons = document.querySelectorAll(
        "div.rcx-box.rcx-box--full.rcx-sidebar-item__wrapper div div figure img.rcx-avatar__element.rcx-avatar--x16"
      );
      const allLinks = document.querySelectorAll(
        "a.rc-box.rcx-box--full.rcx-sidebar-item.rcx-sidebar-item--clickable"
      );
      ...
      allLinks.forEach((e) => {
        ...
        if (
          mainCharacther.classList.contains("rcx-sidebar-item--highlighted")
        ) {
        ...
        if (
          !currE.hasAttribute("data-eventList") &&
          currLab.startsWith("question_")
        ) {
            ...
            const checkExist = setInterval(function () {
              if (
                document.querySelector(
                  "div.rcx-box.rcx-box--full.rcx-css-1yimpo4"
                ) &&
                document.querySelector(
                  ".rcx-box.rcx-box--full.rcx-box--with-inline-elements.rcx-css-1te28na"
                ) &&
                ...
                const extraMesseges = document.querySelector(
                  "div.rcx-box.rcx-box--full.rcx-css-1yimpo4"
                );
                ...
                const forTextArray = {
                  "Нет доступа": `нет доступа к Вашему ноутбуку, откройте, пожалуйста, доступ. Вот ссылка на инструкцию по открытию доступа: https://docs.google.com/document/d/1HXxLTORTUy0aacte6jHFpDfbHisoZys-9TJJQcWKHTA/edit?usp=sharing`,
                ...
                menuButtonField.classList.toggle(
                  "menuButtonField-hidden",
                  true
                );
                ...
                const currField = document.querySelector(
                  ".rcx-box.rcx-box--full.rcx-css-1rdomle"
                );
                ...
                if (
                  document
                    .querySelector('a[rel="noopener noreferrer"]')
                  ...
                  currA.href = document.querySelector(
                ...
                function addMessage(btn) {
                  ...
                  if (currElement) {
                    ...
                    const currTextArea = document.querySelector(
                      'textarea[aria-label="Сообщение"]'
                    );
                    if (!currButton.hasAttribute("data-text")) {
                      currTextArea.value = createMessage(
                        currAttr,
                        pureName,
                        currTimeText
                      );
            ...
            function loopForElement7(someClass) {
              const checkExist = setInterval(function () {
                if (document.querySelector(`[data-toolbox="0"]`)) {
                  const currBtnForInfo =
                    document.querySelector(`[data-toolbox="0"]`);
                  ...
                  function loopForElement4(someClass) {
                    const checkExist = setInterval(function () {
                      if (
                        document.querySelector(
                          `button[type="button"].rcx-box.rcx-box--full.rcx-box--animated.rcx-button--square.rcx-button.rcx-button-group__item.rcx-css-1k1r0f9`
                        )
                      ) {
                        const currMen = document.querySelector(
                          `button[type="button"].rcx-box.rcx-box--full.rcx-box--animated.rcx-button--square.rcx-button.rcx-button-group__item.rcx-css-1k1r0f9`
                        );
                        currMen.click();
                        function loopForElement3(someClass) {
                          const checkExist = setInterval(function () {
                            if (
                              document.querySelector(`ol[role="listbox"] li`)
                            ) {
                              const currMnaa = document.querySelectorAll(
                                `ol[role="listbox"] li`
                              );
                              for (let i = 0; i < 3; i++) {
                                currMnaa[i].style.display = "none";
                              }
                              currMnaa[3].children[0].children[1].classList.add(
                                "BIG"
                              );
                              ...
                              currMnaa[3].children[0].children[1].addEventListener(
                                "mouseover",
                                () => {
                                  loopForElement8(
                                    ".rcx-box.rcx-box--full.rcx-box--animated.rcx-button--primary-danger.rcx-button.rcx-button-group__item"
                                  );
                                }
                              );
                              clearInterval(checkExist);
                  ...
                  function loopForElement5(someClass) {
                    const checkExist = setInterval(function () {
                      if (
                        document.querySelector(
                          `.rcx-box.rcx-box--full.rcx-css-zqz844`
                        )
                      ) {
                        const currCheckbox = document.querySelectorAll(
                          `.rcx-box.rcx-box--full.rcx-css-zqz844`
                        )[1];
                        if (
                          !currCheckbox.children[1].children[0].children[0].hasAttribute(
                            "checked"
                          )
                        ) {
                          currCheckbox.children[1].children[0].click();
                          const currSave = document.querySelector(
                            `button[type="button"].rcx-box.rcx-box--full.rcx-box--animated.rcx-button.rcx-button-group__item.rcx-css-t3n91h`
                          );
                          currSave.click();
                        }

                        const currBack = document.querySelector(
                          `button[type="button"].rcx-box.rcx-box--full.rcx-box--animated.rcx-button--tiny-square.rcx-button--square.rcx-button--ghost.rcx-button.rcx-css-x7bl3q.rcx-css-1yzvz7u`
                        );
                        currBack.click();
                        currBack.click();
                        currBack.click();
                  ...
                  function loopForElement6(someClass) {
                    const checkExist = setInterval(function () {
                      if (
                        document.querySelector(`[aria-label="Редактировать"]`)
                      ) {
                        const currEdit = document.querySelector(
...
margin-right:auto;
    ...
    newBlank.append(
      createLink("ВП", "/admin/app/helpquestion/list", "menu__item"),
      createLink("База", "/lessons", "menu__item")
    );
    ...
    newMenu.append(
      createLink("Админка", "/admin/app/homeworkresult/list"),
      optMenu
    );
    if (menu) {
    ...
    document.querySelectorAll(".customLink").forEach((e) => {
      if (i !== 0) {
        ...
        e.tabIndex = 0;
        document
          .querySelector(".header__info-link")
          .classList.add("customLink-0");
      }
      i++;
    });
    ...
    if (paramsString === "/notifications" || true) {
      ...
      if (currMainPage || currMain) {
        if (
          (currMainPage && paramsString.includes("lesson")) ||
          paramsString.includes("notifications")
        ) {
          currMainPage.style.display = "flex";
        }
        if (
          !currMain.classList.contains("two-column") &&
          !paramsString.includes("lesson")
        ) {
